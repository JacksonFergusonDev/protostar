...
Dry-running only works cleanly on declarative interfaces
...
Related: tri-state toggling

Because modules are independent, Protostar can offer `--<flag>` / `--no-<flag>` overrides for any module without the template author needing to write any conditional logic. See [Initialization](.././usage/init/) for the full flag matrix.
...
The merge behavior is tunable. The default is `MERGE` (preserve your scalars, inject missing nodes). `--force-replace` switches to `OVERWRITE` (Protostar's baseline takes precedence on conflicts). See [The Environment Manifest](.././mechanics/manifest/#collision-strategies) for the full behavior matrix.
...
Expected failures vs. unexpected crashes
...
- **[Why Protostar?](.././why-protostar/):** How these principles compare against generic templaters like Copier in practice.
- **[The Environment Manifest](.././mechanics/manifest/):** Deep dive into the state object that enforces manifest-first execution.
- **[The Orchestrator](.././mechanics/orchestrator/):** How the engine bulkhead and two-phase lifecycle are implemented.
- **[Error Handling Architecture](.././mechanics/error_handling/):** The full exception hierarchy, POSIX routing table, and crash report pipeline.
- **[Agent & Machine Interface](.././usage/agent-interface/):** Driving Protostar programmatically via `--json` and `--dry-run`.
