...
- **All TUI source uses `cli/tui/code.py`.** Render code, structured configuration, and source diffs through its shared renderers and palette. Pass the actual display language when serialization differs from the filename. Language selection is presentation-only; screens never choose their own syntax themes.
