...
The rules live in `protostar/_secret_rules.py`, generated by `scripts/sync_secret_rules.py` from the gitleaks tag pinned for the scaffolded pre-commit hook in `_fallbacks.py`. Regenerate them with `just sync-secret-rules` whenever that tag changes; `just bump` and the release workflow fail while the module is stale.
