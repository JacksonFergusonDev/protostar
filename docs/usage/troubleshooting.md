...
Protostar executes file operations and managed subprocesses within a transaction boundary. If an error occurs during execution or if you interrupt the process (via `Ctrl+C`):

1. **Subprocess Termination:** Protostar immediately signals and terminates any active managed subprocesses.
1. **Automated Rollback:** Protostar restores regular files to their pre-transaction bytes and file modes, and removes empty directories created during the run.
1. **Clean Exit:** Once tracked changes are restored, Protostar raises `PartialExecutionAbortedError` (on `Ctrl+C`) or re-raises the triggering error.

### What Is (and Isn't) Restored

Protostar's rollback is deterministic, transparent, and bounded:

- **What is reliably restored:** All files and directories directly created, written, or AST-merged by Protostar, as well as declared dependency targets (`pyproject.toml` and `uv.lock` journaled before running `uv add`).
- **What might remain:** Protostar does not promise reverting arbitrary, undeclared side effects produced by external commands (such as a `.git/` directory created by `git init` or build caches generated by external tools).

### Partial Rollback Failures (`RollbackFailedError`)

If Protostar cannot completely restore the workspace, it raises `RollbackFailedError` with a list of affected paths. This can happen if:

- **Non-Empty Generated Directories:** An external tool, editor, or process created files inside a directory that Protostar generated. To prevent deleting unrelated user files, Protostar refuses to delete non-empty directories.
- **Filesystem Permissions:** File permissions or ownership were modified during the run, preventing Protostar from writing back the original bytes or removing files.

**Remediation:** Inspect the reported paths listed in the error message and remove or restore them manually before re-running Protostar.

### Unsupported Filesystem Nodes (`UnsupportedFilesystemNodeError`)

Protostar rejects transactional mutations against symbolic links and special filesystem nodes (such as sockets, FIFOs, or device files) to avoid corrupting link targets or following unexpected symlinks outside the workspace.

**Remediation:** Replace the symbolic link or special node with a regular file or directory, or remove it from the workspace before running `protostar init`.
