...
def test_template_initial_and_repeat_merge_convergence(
    ...
    # Whole-file ownership must record the exact generated text, rather than
    # a later user-visible representation. Check this at the complete built-in
    # template matrix boundary, not just in isolated text-merge tests.
    text_records = [
        record for record in state.files if record.policy is FilePolicy.TEXT
    ]
    assert text_records
