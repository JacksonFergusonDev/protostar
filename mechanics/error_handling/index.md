...
- **Clean Parsing:** `stdout` contains only valid JSON. Debug traces and logs are routed exclusively to `stderr`.
- **Structured Fields:** Error objects include `type`, `message`, and optional contextual helpers (`hint`, `docs_url`, and `paths` for collisions).
- **POSIX Status Codes:** The process exits with the exact same POSIX exit code defined in the matrix below, allowing scripts to check either exit codes or the parsed JSON payload.
...
For detailed docstrings and class signatures, see the [Error Handling API Reference](../../developer/api-reference/#class-definitions).

______________________________________________________________________

## Related Guides & References

- **[Troubleshooting & FAQ](../../usage/troubleshooting/):** Remediation steps for missing dependencies, collisions, and editor setups.
- **[Agent & Machine Interface](../../usage/agent-interface/):** Learn how AI coding agents and CI runners parse machine error envelopes.
- **[The Orchestrator](.././orchestrator/):** Understand the top-level exception trap and diagnostic telemetry gathering.
